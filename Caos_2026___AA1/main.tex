...
\journalname{Arthur H. Sandonato - arthur.sandonato@usp.br - 15441240 - \href{https://github.com/asandonato1/Caos2026_AA1/tree/main}{Repositório da atividade}}
...
\rfoot{\includegraphics[height=1cm]{logo.png}} % Logo no topo direito
...
Selecionando o sistema linear
    ...
    Os pontos fixos satisfazem $\vec{\dot{x}} = 0$. Como $\det M \neq 0$, os pontos fixos são $x^* = y^* = 0$.
    ...
    \begin{itemize}
        ...
        \[(M + 2I)\vec{v}_{-2} = 0 \iff \begin{bmatrix}
            -1 & 0 \\
            3 & 0 
        ...
        Similarmente, def. $\vec v_{-3} = \begin{bmatrix}
            u & v
        \end{bmatrix}^T$, temos $(M + 3I)\vec v_{-3} = 0 \iff $
        \[\iff \begin{bmatrix}
            0 & 0 \\
            3 & 1
        \end{bmatrix}\begin{bmatrix}
    ...
    \[\dot{y}_p + 2y_p = 3x_0 e^{-3t} = -3Ke^{-3t} + 2Ke^{-3t} = 3x_0e^{-3t} {\longrightarrow} -3K + 2K = 3x_0 \iff K = -3x_0\]
...
\end{bmatrix} \to \begin{bmatrix}
    1 & 1
\end{bmatrix}$, ou seja, a rotação é no sentido horário. De qualquer forma, o ponto fixo é um centro, ou seja, ainda que estável, não é assintoticamente estável. 
...
\end{bmatrix}, \text{ onde }f'(\theta) = -1 + \frac{\theta^2}{2}\]
    ...
    Podemos escrever a série de Taylor
...
\[\ddot{\theta} = -V'(\theta) \Rightarrow \ddot{\theta}\dot{\theta} + \dot{\theta} V' = 0 \Rightarrow \frac{d}{dt}\left [ \frac{\dot{\theta}^2}{2} + V(\theta)\right ] = 0 \]
...
A diferença entre as duas equações, então, se dá, primeiramente, no termo de ordem $\mathcal{O}(A^4)$. Para pequenas amplitudes, portanto, a aproximação, de fato, vale, sob uma perspectiva de período, também. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{ex3_bif.png}
    \caption{Mapa de bifurcações para $0 < b \leq 2$.}
    \label{fig:ex3_bif}
\end{figure}
\subsection{Expoente de Lyapunov nos regimes periódico e caótico}
A fig. \ref{fig:ex3_bif} apresenta o diagrama de bifurcação para $0 < b \leq 2$ e,
abaixo, o expoente de Lyapunov $\lambda(b)$. Nas regiões periódicas, $\lambda < 0$,
e $\lambda = 0$ nos pontos de bifurcação ($b = 3/4$, $b = 5/4$, \dots); nas regiões
caóticas, onde o atrator preenche intervalos de $x$, $\lambda > 0$.
\subsubsection{Séries temporais}

Os pontos fixos ocorrerão em $x^* = f(x^*) \iff b{x^*}^2+ x^* - 1 = 0 \Rightarrow x^* = \frac{-1 + \sqrt{1 + 4b}}{2b}$, para $x^* \in [-1,1]$. Assim, como $f'(x) = -2bx$, o mapa perde estabilidade quando $|f'(x^*)| > 1$, ou seja, $f'(x^*)= 1 - \sqrt{1 + 4b}$, mas, $f'(x^*) < 1$ $\forall b \in \langle 0, 2]$, então, a única fronteira que pode ser cruzada pelo mapa é em $-1$, que ocorre em $b = 3/4$. Para $3/4 < b < 5/4$, o ciclo de período 2 tem multiplicador
$\mu_2 = f'(x_+)f'(x_-) = 4(1-b)$, com $|\mu_2| < 1$, e é estável.
...
A região sombreada corresponde ao transiente da série periódica ($n < 13$); no caso
caótico, $x_0 = 0{,}1$ já pertence ao atrator $[1-b, 1]$, e não há transiente.
...
Podemos construir o diagrama de cobweb a partir de $x_{n+1} = 1 - bx_n^2$, e, a partir de $(x_0, 0)$, teremos, primeiro, um movimento vertical até a curva, mapeando $(x_n, f(x_n)) \to (x_n, x_{n+1})$, e um horizontal até a diagonal. Para ambos os casos discutidos previamente, temos os mapas da fig. \ref{fig:3aiii}.
...
\begin{figure}[H]
    ...
    \label{fig:3aiii}
\end{figure}
Para $b = 1{,}1$ ($\lambda = -0{,}458$), a teia converge para um retângulo fechado com
vértices em $x_\pm$, o atrator de período 2. Para $b = 1{,}9$ ($\lambda = 0{,}552$),
a teia não se fecha, e as iterações percorrem o intervalo $[1-b, 1]$ sem se repetir.
...
As rotinas abaixo geram as figuras e a tabela apresentadas (as imagens são salvas na pasta \texttt{imagens/}) e também estão disponíveis no \href{https://github.com/asandonato1/Caos2026_AA1/tree/main}{repositório da atividade}.

\subsection*{Ex. 1a) -- \texttt{ex1a.py}}
\addcontentsline{toc}{subsection}{Ex. 1a) -- \texttt{ex1a.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex1a.py}

\subsection*{Ex. 1b) -- \texttt{ex1b.py}}
\addcontentsline{toc}{subsection}{Ex. 1b) -- \texttt{ex1b.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex1b.py}

\subsection*{Ex. 1c) -- \texttt{ex1c.py}}
\addcontentsline{toc}{subsection}{Ex. 1c) -- \texttt{ex1c.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex1c.py}

\subsection*{Ex. 2 -- \texttt{ex2.py}}
\addcontentsline{toc}{subsection}{Ex. 2 -- \texttt{ex2.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex2.py}

\subsection*{Ex. 3 e 3a) -- \texttt{ex3a.py}}
\addcontentsline{toc}{subsection}{Ex. 3 e 3a) -- \texttt{ex3a.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex3a.py}

\subsection*{Ex. 3b) -- \texttt{ex3b.py}}
\addcontentsline{toc}{subsection}{Ex. 3b) -- \texttt{ex3b.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex3b.py}

\subsection*{Ex. 3c)i) -- \texttt{ex3ci.py}}
\addcontentsline{toc}{subsection}{Ex. 3c)i) -- \texttt{ex3ci.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex3ci.py}

\subsection*{Ex. 3c)ii) -- \texttt{ex3cii.py}}
\addcontentsline{toc}{subsection}{Ex. 3c)ii) -- \texttt{ex3cii.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex3cii.py}

\subsection*{Ex. 4 -- \texttt{ex4.py}}
\addcontentsline{toc}{subsection}{Ex. 4 -- \texttt{ex4.py}}
\inputminted[fontsize=\scriptsize, linenos, breaklines, frame=lines]{python}{ex4.py}
